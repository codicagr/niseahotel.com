...
let delayTimer;
$(document).on('input', '#lfm_search_input', function (event) {
  clearTimeout(delayTimer);

  // Ορισμός νέας καθυστέρησης
...
function toggleActions(checkPermissions) {
  ...
  if (checkPermissions) {
    ...
    var working_dir = data['working_dir'];
    var response = performRequest('site/media-permissions-check', {
      'working_dir': working_dir,
      'actions': ['edit', 'delete']
    });
    var hasPermissions = response.responseJSON.hasPermissions;
    if (hasPermissions['edit']) {
  ...
  $('#actionsWithoutSelection').toggleClass('d-none', selected.length !== 0);
...
function goTo(new_dir) {
  ...
  const event = new CustomEvent('updated-working-directory', { detail: new_dir });
...
function performRequest(url, parameter) {
  var data = [];
  if (parameter != null) {
...
function loadItems(page) {
  loading(true);
  performLfmRequest('jsonitems', {search_input: search_input, show_list: show_list, sort_type: sort_type, page: page || 1}, 'html')
    .done(function (data) {
      selected = [];
...
function trash(items) {
  var data = defaultParameters();
  var working_dir = data['working_dir'];
  var response = performRequest('site/media-permissions-check', {'working_dir': working_dir, 'actions': ['delete']});
  var hasPermissions = response.responseJSON.hasPermissions;
  if (!hasPermissions['delete']) {
    Swal.fire(
